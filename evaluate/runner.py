...
def evaluate_configuration(
    ...
    for query_vector in query_vectors:
        ...
        routed_nationality: str | None = None
        routed_clusters: list[int] | None = None
        allowed_speakers: list[str] = []

        if architecture == "metadata_hierarchy":
            routed_gender, routed_nationality = route_metadata_query(query_vector, metric, metadata_centroids)
            if routed_gender is not None:
                if routed_nationality is not None:
                    allowed_speakers = by_gender_nat.get(f"{routed_gender}:{routed_nationality}", [])
                else:
                    allowed_speakers = by_gender.get(routed_gender, [])
        elif architecture == "kmeans_clusters":
            routed_clusters = route_cluster_query(query_vector, metric, cluster_centroids, cluster_probes)
            if routed_clusters:
                speakers_in_clusters: set[str] = set()
                for cluster_id in routed_clusters:
                    speakers_in_clusters.update(cluster_memberships.get(cluster_id, set()))
        ...
        if aggregation == "speaker_centroid":
            ...
            ranked_speakers, top_score = rank_centroids(candidate_ids, candidate_vectors, query_vector, metric, top_k)
        else:
            if architecture == "brute_force":
                utterance_rows = query_utterances(cur, query_vector, metric, candidate_limit)
                ...
                utterance_rows = query_utterances(
                    ...
                    nationality=routed_nationality,
                )
            else:
                utterance_rows = query_utterances(
                    cur,
...
def main() -> None:
    ...
    parser.add_argument("--test-file", type=Path, default=DEFAULT_TEST_EMBEDDINGS)
    parser.add_argument("--max-queries", type=int, default=None)
    parser.add_argument("--metrics", nargs="+", choices=["cosine", "euclidean"], default=list(DEFAULT_METRICS))
    parser.add_argument("--cluster-probes", type=int, nargs="+", default=[1])
    parser.add_argument("--top-k", type=int, default=DEFAULT_TOP_K)
    parser.add_argument("--candidate-limit", type=int, default=DEFAULT_CANDIDATE_LIMIT)
    parser.add_argument("--utterance-mode", choices=["all", "closest", "vote"], default="all")
    ...
    test_vectors, query_speakers, _ = load_test_queries(args.test_file)
    if args.max_queries is not None:
        test_vectors = test_vectors[: args.max_queries]
        query_speakers = query_speakers[: args.max_queries]

    print(f"Loaded {len(test_vectors)} test vectors from {args.test_file}")
    ...
    with psycopg.connect(conn_str) as conn:
        ...
        speaker_index = {speaker_id: index for index, speaker_id in enumerate(speaker_ids)}

        results_by_metric: dict[str, list[dict[str, object]]] = {}
        with conn.cursor() as cur:
            for metric in args.metrics:
                metric_rows: list[dict[str, object]] = []
                for architecture in ["brute_force", "metadata_hierarchy", "kmeans_clusters"]:
                    probe_values = sorted({max(1, value) for value in args.cluster_probes}) if architecture == "kmeans_clusters" else [1]
                    for cluster_probes in probe_values:
                        for aggregation in ["speaker_centroid", "closest", "vote"]:
                            if aggregation != "speaker_centroid" and args.utterance_mode not in {"all", aggregation}:
                                continue
    ...
    if args.output_file:
        save_results(args.output_file, report)
